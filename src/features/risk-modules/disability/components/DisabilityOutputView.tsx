...
function getMonthlyStatsAtAge(outputs: DisabilityOutputs, age: number) {
  ...
  if (!point) {
    ...
    const totalGrossMonthly =
      outputs.ltdComputedMonthlyBenefit + outputs.privateDiMonthlyBenefit
    return {
...
export function DisabilityOutputView({
  ...
  const [visualization, setVisualization] = useState<"coverage" | "module2">(
    "coverage",
  )
  ...
  const ltdDisplayMonthly =
    chartView === "gross" ? monthly.ltdGrossMonthly : monthly.ltdNetMonthly
  const totalDisplayMonthly =
    chartView === "gross" ? monthly.totalGrossMonthly : monthly.totalNetMonthly
  const incomeLossDisplayMonthly =
    chartView === "gross" ? monthly.incomeLossGross : monthly.incomeLossNet
  const assumedIncomeKey =
    chartView === "gross" ? "Assumed Income (Gross)" : "Assumed Income (Net)"
  const assumedIncomeLabel =
    chartView === "gross" ? "Assumed Income (Gross)" : "Assumed Income (Net)"
  const ltdLabel =
    chartView === "gross" ? "Group LTD (Gross)" : "Group LTD (Net)"
  const totalBenefitLabel =
    chartView === "gross" ? "Total Benefit (Gross)" : "Total Benefit (Net)"
  const incomeLossLabel =
    chartView === "gross" ? "Income Loss (Gross)" : "Income Loss (Net)"
  const incomeLossDescription =
    chartView === "gross"
      ? "Assumed gross income minus total gross monthly benefit"
      : "Assumed net income minus total net monthly benefit"

  const totalProjectedIncomeGross = outputs.incomeProjection.reduce(
    (sum, point) => sum + point.annualIncome,
    0,
  )
  const projectedIncomeAtRetirementGross =
    outputs.incomeProjection.at(-1)?.annualIncome ?? 0
  const totalCoverageGross = outputs.incomeProjection.reduce(
    (sum, point) =>
      sum + point.ltdAnnualBenefitGross + point.individualDIAnnualBenefit,
    0,
  )
  const totalGapGross = outputs.incomeProjection.reduce(
    (sum, point) =>
      ...
      Math.max(
        0,
        point.annualIncome -
          (point.ltdAnnualBenefitGross + point.individualDIAnnualBenefit),
      ),
    0,
  )
  const averageCoverageRateGross =
    totalProjectedIncomeGross > 0
      ? totalCoverageGross / totalProjectedIncomeGross
      : 0

  const projectedIncomeDisplay =
    chartView === "gross"
      ? totalProjectedIncomeGross
      : outputs.totalProjectedIncome
  const retirementIncomeDisplay =
    chartView === "gross"
      ? projectedIncomeAtRetirementGross
      : outputs.projectedIncomeAtRetirement
  const totalGapDisplay =
    chartView === "gross" ? totalGapGross : outputs.totalGap
  const averageCoverageRateDisplay =
    chartView === "gross"
      ? averageCoverageRateGross
      ...
      <div className="min-w-[13rem] rounded-lg border border-gray-700 bg-gray-900 p-3 text-sm shadow-lg">
        ...
        <div className="mb-1.5 flex justify-between gap-4">
          ...
          <span className="text-xs font-semibold text-slate-100">
            {formatCurrency(assumedIncomeAnnual)}/yr ·{" "}
            {formatCurrency(assumedIncomeAnnual / 12)}/mo
          </span>
        </div>
        {payload.map((entry: any) => (
          <div key={entry.name} className="mb-1 flex justify-between gap-4">
            ...
            <span className="text-xs font-semibold text-gray-100">
              {formatCurrency(entry.value)}/yr ·{" "}
              {formatCurrency(entry.value / 12)}/mo
            </span>
          </div>
        ))}
      </div>
      ...
      <AnimatedSection delay={0.3}>
        <div className="mb-3 inline-flex max-w-full overflow-x-auto rounded-lg border border-gray-800 bg-[#090E1A] p-1 text-xs">
          <button
            ...
            className={`rounded-md px-3 py-1.5 font-semibold whitespace-nowrap transition-colors ${visualization === "coverage" ? "bg-brand-600 text-white" : "text-gray-400 hover:text-gray-100"}`}
          >
            Annual Coverage
          </button>
          <button
            ...
            className={`rounded-md px-3 py-1.5 font-semibold whitespace-nowrap transition-colors ${visualization === "module2" ? "bg-brand-600 text-white" : "text-gray-400 hover:text-gray-100"}`}
          >
            Break Even
          </button>
          <button
            type="button"
            disabled
            className="rounded-md px-3 py-1.5 font-semibold whitespace-nowrap text-gray-600"
          >
            Module 3
          </button>
        </div>
        ...
        {visualization === "coverage" ? (
          <div className="module-output-container">
            <div
              className={`disability-coverage-grid${formOpen ? "disability-coverage-grid--form-open" : ""}`}
            >
              ...
              <div className="disability-summary-rail">
                ...
                <Card className="module-kpi-card">
                  <CardContent className="p-3.5">
                    <div className="mb-2 text-[10px] font-bold tracking-[0.18em] text-slate-400 uppercase">
                      Lifetime Coverage
                    </div>
                    <div className="divide-y divide-slate-800/80 text-xs">
                      <div className="flex items-center justify-between py-1.5">
                        ...
                        <span className="font-mono font-semibold text-slate-200">
                          {formatCurrency(projectedIncomeDisplay)}
                        </span>
                      </div>
                      <div className="flex items-center justify-between py-1.5">
                        <span className="text-slate-400">Group LTD</span>
                        <span className="font-mono font-semibold text-blue-300">
                          {formatCurrency(outputs.totalGroupLTDCoverage)}
                        </span>
                      </div>
                      <div className="flex items-center justify-between py-1.5">
                        <span className="text-slate-400">Individual DI</span>
                        <span className="font-mono font-semibold text-cyan-300">
                          {formatCurrency(outputs.totalIndividualDICoverage)}
                        </span>
                      </div>
                      <div className="flex items-center justify-between py-1.5">
                  ...
                  <CardContent className="p-3.5">
                    ...
                    <div className="divide-y divide-slate-800/80 text-xs">
                      <div className="flex items-center justify-between py-1.5">
                        ...
                        <span className="font-mono font-semibold text-slate-200">
                          {formatCurrency(retirementIncomeDisplay)}
                        </span>
                      </div>
                      <div className="flex items-center justify-between py-1.5">
                        <span className="text-slate-400">Uncov. Gap</span>
                        <span className="font-mono font-semibold text-red-400">
                          {formatCurrency(totalGapDisplay)}
                        </span>
                      </div>
                      <div className="flex items-center justify-between py-1.5">
                        <span className="text-slate-400">Coverage</span>
                        <span className="font-mono font-semibold text-slate-200">
                          {formatPercent(averageCoverageRateDisplay)}
                        </span>
                      </div>
                      {outputs.lifetimeIDIExpense > 0 && (
                        <div className="flex items-center justify-between py-1.5">
                          ...
                          <span className="font-mono font-semibold text-amber-400">
                            {formatCurrency(outputs.lifetimeIDIExpense)}
                          </span>
                        </div>
                      )}
                    </div>
                  ...
                  <div className="relative flex items-start justify-center">
                    <div className="text-center">
                      ...
                      <p className="mt-1 text-sm leading-snug text-slate-400">
                        LTD and individual DI benefits stacked against projected
                        income need
                      </p>
                      {selectedAge !== null && (
                  ...
                  <div className="flex min-h-0 flex-1 items-stretch gap-1">
                    <div className="flex w-3.5 shrink-0 items-center justify-center">
                      <span
                        ...
                        className="text-[10px] font-semibold tracking-wider whitespace-nowrap text-slate-500 uppercase"
                      >
                        Annual Benefit ($)
                      </span>
                    </div>
                    <div className="flex min-w-0 flex-1 flex-col">
                        ...
                        >
                          <BarChart
                            ...
                            onClick={(data) => {
                              if (data?.activePayload)
                                setSelectedAge(Number(data.activeLabel))
                            }}
                          ...
                          >
                            <XAxis
                              dataKey="age"
                              tick={({ x, y, payload }) => {
                                const ages = chartData.projectionChartData.map(
                                  (d) => d.age,
                                )
                                const step = Math.ceil(ages.length / 8)
                                const showTick =
                                  ages.indexOf(payload.value) % step === 0 ||
                                  payload.value === ages.at(-1)
                                return showTick ? (
                                  <text
                                    x={x}
                                    y={y + 12}
                      ...
                      <div className="mt-2 flex flex-wrap items-center justify-center gap-x-4 gap-y-1">
                        ...
                        <span className="inline-flex items-center gap-1.5 text-[11px] text-gray-400">
                          <span className="inline-block h-2.5 w-4 rounded-sm bg-[#06b6d4]" />
                          Individual DI
                        </span>
                        <span className="inline-flex items-center gap-1.5 text-[11px] text-gray-400">
              ...
              <div className="module-metric-rail">
                <MetricGroup title="Monthly Benefits">
                  <ModuleMetricCard
                    ...
                    value={
                      <>
                        {formatCurrency(ltdDisplayMonthly)}
                    ...
                    description={
                      chartView === "gross"
                        ...
                        : "Net after-tax LTD monthly benefit"
                    }
                    accent="blue"
                  />
                  <ModuleMetricCard
                    ...
                    value={
                      <>
                        {formatCurrency(monthly.individualDIMonthly)}
                        <span className="text-sm font-normal text-gray-400">
                          /mo
                        </span>
                      </>
                  ...
                  <ModuleMetricCard
                    ...
                    value={
                      <>
                        {formatCurrency(totalDisplayMonthly)}
                        <span className="text-sm font-normal text-gray-400">
                          /mo
                        </span>
                      </>
                ...
                <MetricGroup title="Gap">
                  <ModuleMetricCard
                    ...
                    value={
                      <>
                        {formatCurrency(incomeLossDisplayMonthly)}
                        <span className="text-sm font-normal text-gray-400">
                          /mo
                        </span>
                      </>
        ...
        <AnimatedSection delay={0.46}>
          <Card className="border border-gray-800 bg-[#090E1A] text-white">
            <CardContent className="p-6">
              <h4 className="mb-2 text-xs font-semibold tracking-wider text-blue-400 uppercase">
                Planning Narrative
              </h4>
